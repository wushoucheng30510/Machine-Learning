...
for parameter in regu_par:
    ...
    while (k<iterations):
        ...
        for j in range (0,len(w)-1):
             w[j] = (np.sign (w[j])) * (max((np.abs(w[j]) - learning_rate * regu_par[index_k]), 0))
        ...
        for i in range (0,len(result)):
            if result[i]==0:
                acc +=1
        #print ("Iterations:",k, "          Current Accuracy",acc/len(result))
...
plt.title("Accuracy v.s Regulariztion parameter  (learning rate =0.01, iterations =2500) ")
